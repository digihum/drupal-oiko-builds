 ...
 *
 * @ViewsRelationship("cidoc_related_entity_forward")
 */
class CIDOCEntity extends RelationshipPluginBase {
   ...
   */
  public function __construct(array $configuration, $plugin_id, $plugin_definition, ViewsHandlerManager $join_manager, EntityTypeBundleInfoInterface $entityBundleInfo) {
  ...
  public function query() {
    ...
    $domain_views_data = Views::viewsData()->get('cidoc_reference__domain');
//    $domain_left_field = $views_data['table']['base']['field'];

    $first = array(
      'left_table' => $this->tableAlias,
      'left_field' => 'id',
      'table' => 'cidoc_reference__domain',
      'field' => 'domain_target_id',
      'adjusted' => TRUE
    );
    ...
    $this->first_alias = $this->query->addTable('cidoc_reference__domain', $this->relationship, $first_join);
    ...
    $alias = $this->first_alias . '__range';

    $this->second_alias = $this->query->addRelationship($alias, $second_join, 'cidoc_reference__range', $this->relationship);

    // Finally add our target table.

    $third = array(
      'left_table' => $this->second_alias,
      'left_field' => 'range_target_id',
