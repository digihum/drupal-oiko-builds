...
class CidocEntityForm extends ContentEntityForm {
    ...
    if ($applicable_properties = $cidoc_entity->bundle->entity->getAllEditableProperties($reverse)) {
      ...
      foreach ($applicable_properties as $property_name => $property_entity) {
        ...
        if ($form_state->get('form_display')->getComponent($element_key)) {
          ...
          $headers = array();
          // For the target field, use the title of the target's own label property.
          if ($label_key = $cidoc_entity_type->getKey('label')) {
            if (isset($form[$label_key]['widget']['#title'])) {
              $headers[$target_field] = clone $form[$label_key]['widget']['#title'];
            }
          }
              ...
              if (isset($form[$element_key]['references'][$reference_id]['subform'][$target_field]['widget']['target_id'])) {
                ...
                if ($existing_reference) {
                  ...
                  $target_field_element['existing_reference'] = array(
                    '#type' => 'item',
                    '#markup' => $target_entity->label() . ' &mdash; ' . Link::fromTextAndUrl(
  ...
  public function save(array $form, FormStateInterface $form_state) {
    ...
    drupal_set_message($this->t('Saved the %label CIDOC entity.', ['%label' => $entity->label()]));
    $form_state->setRedirect('entity.cidoc_entity.canonical', ['cidoc_entity' => $entity->id()]);
  }
