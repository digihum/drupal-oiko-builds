...
exports.RECEIVE_CIDOC_ENTITY = exports.REQUEST_CIDOC_ENTITY = undefined;
exports.fetchFetchCidocEntityIfNeeded = fetchFetchCidocEntityIfNeeded;
exports.cidocEntityReducer = cidocEntityReducer;
exports.connectSidebar = connectSidebar;

var _reduxHistory = __webpack_require__(1);
 ...
 * }
 */
var REQUEST_CIDOC_ENTITY = exports.REQUEST_CIDOC_ENTITY = 'oiko/REQUEST_CIDOC_ENTITY';
...
var RECEIVE_CIDOC_ENTITY = exports.RECEIVE_CIDOC_ENTITY = 'oiko/RECEIVE_CIDOC_ENTITY';
...
function fetchCidocEntity(id) {
  return function (dispatch) {
    ...
    return Drupal.oiko.displayContentInLeafletSidebar(id, function () {
...
function shouldFetchCidocEntity(state, id) {
  var cidocState = state.cidocEntity;
  if (!cidocState) {
    return true;
...
function fetchFetchCidocEntityIfNeeded(id) {
...
function cidocEntityReducer() {
  ...
  switch (action.type) {
    ...
    case _reduxHistory.UPDATE_LOCATION:
      return Object.assign({}, state, {
        isFetching: false,
        id: Number.parseInt((0, _querystringHelpers.fetchQueryStringElements)(action.payload, _querystringDefinitions.QUERYSTRING_VARIABLE_SIDEBAR_CIDOC_ENTITY, state.id), 10)
      });
...
function connectSidebar($, store) {

  var currentCidocEntity = void 0;
  var lastDispatch = void 0;

  var checkStoreState = function checkStoreState() {
    var _store$getState = store.getState(),
        cidocEntity = _store$getState.cidocEntity;
  ...
  $(window).on('oiko.loaded', function () {
    checkStoreState();
 ...
 * @returns {oikoApp}
 */
function createOikoApp(jQuery) {
  return new oikoApp(jQuery);
}

var oikoApp = function () {
  function oikoApp(jQuery) {
    var _this = this;

    _classCallCheck(this, oikoApp);
    ...
    this.store = (0, _redux.createStore)(_reducers2.default, (0, _redux.applyMiddleware)(_reduxThunk2.default, loggerMiddleware));

    var history = (0, _createBrowserHistory2.default)();
    (0, _reduxHistory.connectHistory)(history, this.store);
    (0, _sidebar.connectSidebar)(this.$, this.store);

    // Fetch the location, on window load, we'll put it back into our state.
    this.initialLocation = history.location;

    this.store.dispatch((0, _reduxHistory.updateLocation)(history.location));

    this.initFired = false;

    var checkInitState = function checkInitState() {
      if (_this.initFired) {
        return;
      }
      var initDone = true;
      ...
      var _store$getState = _this.store.getState(),
          appModules = _store$getState.appModules;
      ...
      try {
        ...
        for (var _iterator = Object.keys(appModules)[Symbol.iterator](), _step; !(_iteratorNormalCompletion = (_step = _iterator.next()).done); _iteratorNormalCompletion = true) {
          var key = _step.value;
      ...
      if (initDone) {
        _this.$(window).trigger('oiko.loaded', appModules);
        _this.initFired = true;
      }
    };
...
var app = (0, _store.createOikoApp)(jQuery);
...
Drupal.oiko.addAppModule = function (moduleName) {
  return store.dispatch((0, _actions.addAppModule)(moduleName));
};

Drupal.oiko.appModuleDoneLoading = function (moduleName) {
  return store.dispatch((0, _actions.appModuleDoneLoading)(moduleName));
};
